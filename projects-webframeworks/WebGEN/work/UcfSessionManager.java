...
// Source File Name:   UcfSessionManager.java
...
public class UcfSessionManager
    implements Serializable
{
    ...
    UcfSessionManager(File workingDir)
    {
        m_workingDir = workingDir;
        if(Trace.UCF_MANAGER)
            Trace.println("created: " + this);
    }
    ...
    public synchronized IServerSession getSession()
    {
        ...
        if(m_connectedSessions.isEmpty())
        {
            ...
            if(session != null)
            {
                ...
                if(Trace.UCF_MANAGER)
                    Trace.println("get new session: " + session.getUID() + ", refcount: " + rc.getCount());
            }
        } else
        {
            for(int i = 0; i < m_connectedSessions.size(); i++)
            {
                ...
                if(rc.getCount() == 1)
                    reserveConnectedSession(session);
                if(Trace.UCF_MANAGER)
                    Trace.println("get connected session: " + session.getUID() + ", refcount: " + rc.getCount());
            }
    ...
    public synchronized void release(IServerSession session)
    {
        ...
        rc.dec();
        if(Trace.UCF_MANAGER)
            Trace.println("release session: " + session.getUID() + ", refcount: " + rc.getCount());
        if(rc.getCount() == 0)
        {
    ...
    synchronized void addNewId(String sessionStr, String msg)
    {
        ...
        notifyAll();
        if(Trace.UCF_MANAGER)
            Trace.println("new session id added: " + sessionStr);
    }
    ...
    synchronized void reserveSession(String requestKey)
    {
        ...
        if(Trace.UCF_MANAGER)
            Trace.println("reserved " + requestKey + " by " + Thread.currentThread());
    }
    ...
    synchronized void unreserveSession()
    {
        ...
        if(Trace.UCF_MANAGER)
            Trace.println("unreserved " + requestKey + " by " + cthread);
    }
        ...
        } else
        {
            try
            {
                ...
                for(long waitTime = 60000L; (index = m_unusedSessions.indexOf(sessionKey)) == -1; waitTime = 60000L - (System.currentTimeMillis() - start))
                {
                    ...
                    if(Trace.UCF_MANAGER)
                        Trace.println("newSession: waiting for session...");
                    wait(waitTime);
                }
    ...
    private IServerSession newSession(String sessionId)
    {
        ...
        if(session != null)
        {
            ...
            if(Trace.UCF_MANAGER)
                Trace.println("created new session: " + session + ", id: " + session.getUID());
            if(m_locale != null)
    ...
    private void disconnectSession(IServerSession session)
    {
        ...
        {
            Trace.println("failed disconnecting session: " + id);
            e.printStackTrace();
        }
        finally
        {
            ...
            if(Trace.UCF_MANAGER)
                Trace.println("session disconnected: " + id);
        }
    }
    ...
    private void reserveConnectedSession(IServerSession session)
    {
        ...
        if(Trace.UCF_MANAGER)
            Trace.println("reserved " + session + " by " + cthread);
    }
    ...
    private void unreserveConnectedSession(IServerSession session)
    {
        ...
        if(Trace.UCF_MANAGER)
        {
            ...
            Trace.println("unreserved " + session + " by " + currentThread + (reservingThread == currentThread ? "" : " reserved by " + reservingThread));
        }
    }
