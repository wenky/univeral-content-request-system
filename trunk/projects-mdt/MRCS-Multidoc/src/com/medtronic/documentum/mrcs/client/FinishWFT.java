    ...
    public boolean onCommitChanges() {
        ...
        try {
            ...
            if (super.canCommitChanges() && preCheck.isTaskEffectual(map, preCheckClass.PluginConfiguration) && getValid()) {
                ...
                canCommit = false;
                // check signing failure reason...added this for MSD CAS b/c it's workflow is unique - I think all other signings use ForwardWFT, but b/c there is no reject 
                // reason for MSD CAS's accept tasks, it uses FinishWFT
                ...
                if (canCommit) {
                    ...
                    if (docObject.isSuspended()) {
                        ...
                        if (config.isLegacyLCWF(appName)) {
                        	...
                        	IDfId workflowid = iworkflowtask.getWorkflowId();
                        	...
                        	MrcsWorkflowTask mrcstask = config.getMrcsWorkflowTask(appName,wfdef.getObjectName(),iworkflowtask.getTaskName());
                        	if (mrcstask.Actions != null) 
                        	{
                                ...
                                for (int k=0; k < mrcstask.Actions.size(); k++)
                                {
                                	try { 
	                                    ...
	                                    wfaction.execute(getDfSession().getSessionManager(),getDfSession().getDocbaseName(), iworkflowtask, workflow, appName, plugin.PluginConfiguration, context);
                                	} catch (Exception e) {
