...
public class ViewPDFRendition extends LaunchComponentWithPermitCheck
{
     ...
     *
     */
    protected boolean hasPDFRendition(String objectId)
    {
        try
        {
            ...
            IDfCollection contentTypes = sysObject.getRenditions("r_object_id");

            while(contentTypes.next())
            {
                String attrValue = contentTypes.getString("r_object_id");
                IDfPersistentObject persObj = (IDfPersistentObject)
                                               docbaseSession.getObject(new DfId(attrValue));
                String format = persObj.apiGet("get","full_format");
