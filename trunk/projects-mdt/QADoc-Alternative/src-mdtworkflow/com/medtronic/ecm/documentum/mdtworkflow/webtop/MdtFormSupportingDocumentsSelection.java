...
public class MdtFormSupportingDocumentsSelection extends ObjectGrid
{
    ...
    public MdtFormSupportingDocumentsSelection()
    {
    ...
    protected String getQuery(String strVisibleAttrs, ArgumentList args)
    {
        ...
        try { 
            ...
            String query = "SELECT r_object_id,title,object_name,r_link_cnt,r_is_virtual_doc,owner_name,r_object_type,a_content_type,r_lock_owner,r_content_size,i_is_reference,r_assembled_from_id," +
                           ...
                           "FROM mdt_qad_doc " +
                           "WHERE m_application = '"+m_mdtapp+"' ORDER BY object_name";
    ...
    {
        Datagrid docselection = (Datagrid)this.getControl(MdtFormSupportingDocumentsSelection.GRID_NAME,Datagrid.class);
    ...
    public void onAttachmentOk(Button control, ArgumentList args)
    {
        try { 
            ...
            AttachmentUtils.removeAllDocumentsFromDocumentAttribute(so, getDocAttr(), getRemoveKey());
            Iterator i = m_selecteddocuments.keySet().iterator();
            AttachmentUtils.addDocumentsToDocumentAttribute(so, i, getDocAttr(), getAddKey());            
        } catch (DfException e) {
