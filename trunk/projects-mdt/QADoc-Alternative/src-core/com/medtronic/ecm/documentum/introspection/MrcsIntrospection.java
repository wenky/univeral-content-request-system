...
// helper class to generate and process dumps for introspection of MRCS...
...
public class MrcsIntrospection
{
	...
	public String perform(HttpServletRequest request) throws Exception
	{
		...
		if ("viewcontent".equals(page))
			return showContent(request);		
	...
	public String showContent(HttpServletRequest request) throws Exception
	{
		String objectid = request.getParameter("objectid");
		IDfSession session = this.getSession();
		String thedoc = null;
		try {
			IDfSysObject so = (IDfSysObject)session.getObject(new DfId(objectid));
	...
	public String manipulateAttributes(HttpServletRequest request) throws Exception
	{
		...
		try {
			...
			Collections.sort(attrlist, new Comparator() { 
				public int compare(Object o1, Object o2)  { return ((IDfAttr)o1).getName().compareTo(((IDfAttr)o2).getName());}
			});
	...
	public String dumpFolder(String folderid) throws Exception
	{
        ...
        html += generateQueryTable(sysdql); 
        html += "</div>";
	...
	public String dumpDocument(String docid) throws Exception
	{
		...
		// view content link
		html += "<hr>"+baselink+"&page=viewcontent&objectid="+docid+"\">View Content</a><hr>";
	...
	{
		IDfSession session = this.getSession();
		IDfTypedObject t = (IDfTypedObject)session.getObject(new DfId(id));
		String dump = t.dump();
