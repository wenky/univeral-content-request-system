...
public class PackageHttpParameters
{
    
    //  this worked on the FIRST try (aside from bullshit ArrayList not having a setSize() method)
    static void packageParameter(ServletRequest req, String parameter, Object collection, int pointer)
    {
        ...
        if (Character.isLetter(firstchar))
        {
            ...
            else
            {
                ...
                if (nextcollection == null)
                {
                    ...
                    if (Character.isLetter(parameter.charAt(pointer+nextDot+1)))
                    {
                        nextcollection = new HashMap();
                    }
                    else if (Character.isDigit(parameter.charAt(pointer+nextDot+1)))
                    {
    ...
    public static Object exec(String prefix, ServletRequest req) 
    {
        String evalprefix = prefix;        
            ...
            if (current.length() >= evalprefix.length())
            {
                if (current.substring(0,evalprefix.length()).equals(evalprefix))
                {
                    ...
                    if (collection == null)
                    {
                        ...
                        if (Character.isLetter(packtype))
                        {                   
                            // Map...
                            collection = new HashMap();
                        }
                        else if (Character.isDigit(packtype))
                        {
                            // List...
