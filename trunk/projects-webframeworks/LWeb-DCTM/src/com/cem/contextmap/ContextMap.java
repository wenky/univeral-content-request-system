...
import com.cem.contextmap.web.MapWrapper;
...
public class ContextMap implements Map
{
    ...
    public IContextItem getNamedContext(String name) 
    {
        Integer idx = (Integer)getPrecedenceNames().get(name);
        if (idx == null) return null;
        Object subcontext = getPrecedence().get(idx);
        if (subcontext instanceof Map) return new MapWrapper((Map)subcontext);
        else return (IContextItem)subcontext;
    }
