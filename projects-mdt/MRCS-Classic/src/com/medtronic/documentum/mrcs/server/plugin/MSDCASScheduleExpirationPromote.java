...
public class MSDCASScheduleExpirationPromote implements IMrcsWorkflowServerPlugin, IMrcsLifecyclePlugin
{
	...
	public void execute(IDfSessionManager sMgr, String docbase, MrcsLifecycleState targetstate,String mrcsapp,IDfSysObject mrcsdocument,Map config, Map context)
	{
    	/*-CONFIG-*/String m="execute - ";
       	...
       	try {
			...
			throw new RuntimeException("Error in Lifecycle plugin - MSDCAS schedule expiration on server event plugin",dfe);
    	...
    	/*-CONFIG-*/String m="execute - ";
       	...
       	try {
		    ...
		    try { 
	       	...
	       	/*-ERROR-*/DfLogger.error(this, m+"ERROR in generate rendtion on server event" , null, dfe);
			throw new RuntimeException("Error in workflow plugin - MSDCAS schedule expiration on server event plugin",dfe);
       	} finally { sMgr.release(session); }
    ...
    public void startScheduledPromote(IDfSession session, IDfDocument docObject)
    {
        ...
        /*-DEBUG-*/if (DfLogger.isDebugEnabled(this)) DfLogger.debug(this,m+"MSDCASScheduleExpirationPromote.startScheduledPromote called.",null,null);

        try {
            IDfTime dt = docObject.getTime("expiration_date"); 
            ...
            {            
	            ...
	            String jobname = "MSDCASScheduledExpiration-"+docObject.getObjectId().getId()+"-"+dt.toString();
