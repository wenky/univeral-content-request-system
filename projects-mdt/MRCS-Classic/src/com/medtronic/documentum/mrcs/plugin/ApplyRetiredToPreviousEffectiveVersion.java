...
public class ApplyRetiredToPreviousEffectiveVersion extends ThereCanOnlyBeOne
{
    ...
    {
        /*-CFG-*/String m = "ApplyRetiredToPreviousEffectiveVersion.getDQL()-";
        ...
        // assume the doctype has retired/obsolete - get the underlying doctype so we can run the proper query
        String doctype = PostVersionedDoc.getTypeName();
        /*-DEBUG-*/if (DfLogger.isDebugEnabled(this))DfLogger.debug(this, m+"underlying doctype: "+doctype, null, null);
               
        /*-DEBUG-*/if (DfLogger.isDebugEnabled(this))DfLogger.debug(this, m+"compose DQL for looking up both newer and older versions of the current doc that are not retired", null, null);
        String qualification = "select r_object_id from "+doctype+" (ALL) "+
                               "where NOT r_object_id = '"+docid+"' AND r_current_state = "+curstate+" AND retired = false AND i_chronicle_id in " +
    ...
    // apply the specified ACL to all matches, flip retired flags
    ...
    {
        /*-CFG-*/String m = "ApplyRetiredToPreviousEffectiveVersion.process()-";
        /*-DEBUG-*/if (DfLogger.isDebugEnabled(this))DfLogger.debug(this, m+"reacquire session", null, null);
        IDfSession session = sMgr.getSession(docbase);
