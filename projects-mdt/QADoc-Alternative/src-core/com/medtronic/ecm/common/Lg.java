...
/**
 * Medtronic and Documentum adapted version of the google code log5j library, which uses printf-style
 * message formatting and varargs, but wraps around lgger rather than Logger. Provides several 
 ...
 *  MdtLog.ftl("test (static) a simple msg");
 * 
 *  // simple message formatting 
 *  MdtLog.ftl("test (static) a simple substitution: %s","a-string");
 * 
 *  // simple message format + exception trace log
 *  Exception e = new Exception("BigBadError");
 *  MdtLog.ftl("test (static) sub+stacktrace: %d",4,e);
 *
 * </pre></blockquote>
 * 
 ...
 * 
 *      MdtLog.dbg("Hi %s, %s",lastname,firstname);
 *      
 * since the formatting occurs inside the MdtLog.dbg method. HOWEVER this still entails overhead:
 * 
 *     MdtLog.dbg("Name of object: %s",sysobject.getObjectName());
 * 
 ...
 * 
 *     if (MdtLog.dbg())MdtLog.dbg("Name of object: %s",sysobject.getObjectName());
 *     
 * or rely on the objectlogger feature in the MdtLog. 
 *     
...
public class Lg 
{
    ...
    // static initializer (load objectlogger config from resource bundles
    public static List ObjectReplacers = null;
    static {
        ObjectReplacers = new ArrayList();
        try { 
            ResourceBundle objectloggerconfig = ResourceBundle.getBundle("com.medtronic.ecm.common.LgObjectLoggers");
            if (objectloggerconfig != null) {
                List handlers = null;
                List contexts = null;
                boolean proceed = true;
                // if classlist was found, get handler list (must be same size as class list)
                
                try {
                    int hdlcount = 0;
                    while (true) {
                        String curhandlername = objectloggerconfig.getString("Lg.objectlogger.handler["+hdlcount+"]");
                        if (curhandlername == null) break; // missingresource is more likely, but whatever
                        if (handlers == null) handlers = new ArrayList(); // lazy init so we can flag whether to proceed.
                        handlers.add(curhandlername);
                        hdlcount++;
                    }
                } catch (MissingResourceException mse) {
                    // this is the exit condition, not that I have a choice...
                }
                if (handlers == null)
                    proceed = false;
                if (proceed) {
                    try {
                        ...
                        for (int i=0; i < handlers.size(); i++){
                            String curcontext = null;
                            try { curcontext = objectloggerconfig.getString("Lg.objectlogger.context["+i+"]"); } catch (MissingResourceException mre) {}
                            contexts.add(curcontext);
                        }
                        
                    } catch (MissingResourceException mse) {
                        // contexts are totally optional on a per-objectlogger basis, so we can ignore this
                    }
                    
                    for (int i=0; i < handlers.size(); i++) {
                        String handlername = (String)handlers.get(i);
                        try {
                            Class objectloggerclass = Class.forName(handlername);
                            ILgObjectSerializer objectlogger = (ILgObjectSerializer)objectloggerclass.newInstance();
                            ObjectReplacers.add(objectlogger);
                            if (contexts != null && contexts.size() > i) {
                                String contextbundle = (String)contexts.get(i);
                                try { 
                                    if (contextbundle != null) {
                                        ResourceBundle context = ResourceBundle.getBundle(contextbundle);
                                        objectlogger.setContext(context);
                                    }
                                } catch (MissingResourceException mse) {
                                    // contexts are totally optional on a per-objectlogger basis, so we can ignore this
                                }
                            ...
                            System.out.println("com.medtronic.ecm.common.Lg :: WARNING: Error in instantiation of objectlogger handlers");                                               
                        }
                    }                    
                }
            }
        } catch (MissingResourceException mse) {
            System.out.println("com.medtronic.ecm.common.Lg :: WARNING: Object logger configuration resource not found");
        } catch (Exception e) {
            System.out.println("com.medtronic.ecm.common.Lg :: WARNING: Encountered an error configuring object loggers");            
        }
    }
     ...
     * 
     */
    public static void trc(String format, Object... args) { log(Level.TRACE,format,args); }
    public static void dbg(String format, Object... args) { log(Level.DEBUG,format,args); }
    public static void inf(String format, Object... args) { log(Level.INFO,format,args); }
    public static void wrn(String format, Object... args) { log(Level.WARN,format,args); }
    public static void err(String format, Object... args) { log(Level.ERROR,format,args); }
    public static void ftl(String format, Object... args) { log(Level.FATAL,format,args); }
    
    public static void log(Level lvl,String format, Object... args)
    {
     ...
     * 
     */
    public static void trcUtil(int stackdepth, String format, Object... args) { logUtil(stackdepth+1,Level.TRACE,format,args); }
    public static void dbgUtil(int stackdepth, String format, Object... args) { logUtil(stackdepth+1,Level.DEBUG,format,args); }
    public static void infUtil(int stackdepth, String format, Object... args) { logUtil(stackdepth+1,Level.INFO,format,args); }
    public static void wrnUtil(int stackdepth, String format, Object... args) { logUtil(stackdepth+1,Level.WARN,format,args); }
    public static void errUtil(int stackdepth, String format, Object... args) { logUtil(stackdepth+1,Level.ERROR,format,args); }
    public static void ftlUtil(int stackdepth, String format, Object... args) { logUtil(stackdepth+1,Level.FATAL,format,args); }

    public static void logUtil(int stackdepth, Level lvl, String format, Object... args)
    {
        // stack object detection could entail high overhead for logging...
        String curclass = "UnknownClass ";
        String methodname = "unknownmethod";
    ...
    public static void replaceObjectsWithInfo(Object[] args)
    {
        if (args != null)
        {
            for (int i=0; i < args.length; i++) 
            {
                ...
                for (int j=0; j < ObjectReplacers.size(); j++)
                {
                    ILgObjectSerializer objlogger = (ILgObjectSerializer)ObjectReplacers.get(j);
                    Class matchclass = objlogger.getMatchingClass();                    
                    if (matchclass.isInstance(args[i])) {
                        try {
                            String dump = objlogger.replaceObject(args[i]);
                            args[i] = dump;
                        } catch (Exception e) {}
                    }
                }
    ...
    // compatibility - assume log4j for now
