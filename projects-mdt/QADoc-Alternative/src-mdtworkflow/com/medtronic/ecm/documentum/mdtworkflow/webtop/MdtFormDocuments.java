...
public class MdtFormDocuments extends ObjectGrid
{
    ...
    public void onInit(ArgumentList args)
    {
        ...
        try { 
            ...
            if (formdoc.getValueCount("m_attachments") == 1 && "".equals(formdoc.getRepeatingString("m_attachments",0).trim())) {
                formdoc.remove("m_attachments", 0);
                formdoc.save();
            }
        } catch (DfException dfe) {
        ...
        if ("multi".equals(args.get("objectIds"))) {
            ...
            if (compargs != null) {
                ...
                for (int b=0; b < compargs.length; b++) {
                    ...
                    AttachmentUtils.addAttachmentToForm(formdoc, curid);
                }                    
            }
        }
        ...
        if (!isEditable()) {
            Button b = (Button)getControl("addAttachment",Button.class);
            b.setVisible(false);
            b.setEnabled(false);
        }
    ...
    {        
        try {                        
            ...
            String query = "SELECT r_object_id,object_name,title,r_link_cnt,r_is_virtual_doc,owner_name,r_object_type,a_content_type,r_lock_owner,r_content_size,i_is_reference,r_assembled_from_id," +
                           ...
                           "FROM mdt_document " +
                           "WHERE r_object_id in (select r_object_id from mdt_document where object_name in (select m_attachments from mdt_workflow_form(all) where r_object_id = '"+m_strObjectId+"')) ";            
    ...
    public void onCompleteAddDocument(Button control, ArgumentList args){
        try { 
            ...
            IDfSysObject formdoc = (IDfSysObject)getDfSession().getObject(new DfId(m_strObjectId));
            String[] idlist = new String[formdoc.getValueCount("m_attachments")];
        ...
        setComponentNested("mdtformdocumentsselection", args, getContext(), this);
